...
<div class="row">
    <div class="col-12">
        <div class="card-box">
            <h4 class="mt-0 mb-2 header-title">Edit Mata Kuliah</h4>
            <div class="p-2">
                <form action="{{ route('subject.update', ['id' => $subject->id]) }}" method="POST"
                    class="form-horizontal" role="form">
                    @csrf
                    <div class="form-group row">
                        <label class="col-md-2 col-form-label" for="simpleinput">Judul Mata Kuliah</label>
                        <div class="col-md-10">
                            <input name="curriculum" type="text" id="simpleinput"
                                class="form-control @error('curriculum') is-invalid @enderror"
                                placeholder="Masukkan Judul Mata Kuliah" value="{{ $subject->curriculum }}">
                            @error('curriculum')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <textarea name="subject" class="form-control @error('subject') is-invalid @enderror"
                                rows="5" id="editor">{{ $subject->subject }}</textarea>
                            @error('subject')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
